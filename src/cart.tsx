...
import { fetchProduct, Product } from "./product-list";

const Cart = () => {
	const { cart, setCart, shopCount, setShopCount, setCartCount } =
	...
	return (
		<div id="container" className="w-full">
			...
			{cart.length === 0 ? (
				...
				<div id="cart" className="grid grid-cols-5 gap-5 max-md:grid-cols-3">
					{cart.map((item) => (
						<div
							key={item}
							id={item}
						...
						>
							<img
								src={products?.[item - 1].image}
								alt="product-image"
								className="w-full"
							/>
							<p className="opacity-45 px-2">{products?.[item - 1].category}</p>
							...
							<p className="px-2">Quantity: {shopCount[item]}</p>
